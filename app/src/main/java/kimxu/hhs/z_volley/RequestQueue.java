...
import kimxu.hhs.z_volley.http.AbstractNetworkQueue;
 ...
 */
public class RequestQueue implements AbstractNetworkQueue {
	 ...
	 */
	private final Map<Long, Request<?>> mCurrentRequests = CollectionBuilder
			.newHashMap();
	...
	public RequestQueue(Cache cache, Network network, int threadPoolSize,
		...
		mDelivery = delivery;
		// requests = CollectionBuilder.newHashMap();
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.app.china.framework.util.volley.AbstractNetworkQueue#start()
	 */
	@Override
	public void start() {
		...
		for (int i = 0; i < mDispatchers.length; i++) {
			NetworkDispatcher networkDispatcher = new NetworkDispatcher(
					mNetworkQueue, mNetwork, mCache, mDelivery, i);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.app.china.framework.util.volley.AbstractNetworkQueue#stop()
	 */
	@Override
	public void stop() {
	...
	public void cancelAll(RequestFilter filter) {
		synchronized (mCurrentRequests) {
			Collection<Request<?>> reqs = mCurrentRequests.values();
	...
	public <T> Request<T> add(Request<T> request) {
		...
		synchronized (mCurrentRequests) {
			mCurrentRequests.put(request.getId(), request);
		}
...
//				L.v("Request for cacheKey=%s is in flight, putting on hold.",
//						cacheKey);
	...
	void finish(Request<?> request) {
		...
		synchronized (mCurrentRequests) {
			mCurrentRequests.remove(request.getId());
		}
...
//
//					L.v("Releasing %d waiting requests for cacheKey=%s.",
