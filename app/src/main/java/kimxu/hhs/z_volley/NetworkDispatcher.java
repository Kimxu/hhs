...
public class NetworkDispatcher extends Thread {
	private static final String THREAD_NAME = "network_dispatcher_";
	...
	public NetworkDispatcher(BlockingQueue<Request<?>> queue, Network network,
			Cache cache, ResponseDelivery delivery, int index) {
		mQueue = queue;
		mNetwork = network;
		mCache = cache;
		mDelivery = delivery;
		this.setName(THREAD_NAME + index);
	}
	...
	private void addTrafficStatsTag(Request<?> request) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
			...
			android.support.v4.net.TrafficStatsCompat.setThreadStatsTag(request
					.getTrafficStatsTag());
		}
	}
	...
	public void run() {
		...
		while (true) {
			try {
				...
				request = mQueue.take();
				// L.x("network dispatcher", "handle a task ", request.getUrl(),
				// request.getId());
			...
			try {
				...
				// L.x("network dispatcher", "finish the request",
				// "start dispatch", response.isSuccess());
