...
public class ImageRequest extends Request<Bitmap> {
	...
	private static final float IMAGE_BACKOFF_MULT = 2.5f;
	...
	public ImageRequest(String url, int maxWidth, int maxHeight,
			Config decodeConfig, final CommonResponceListener<Bitmap> ls) {
	 ...
	 */
	protected static int getResizedDimension(int maxPrimary, int maxSecondary,
	...
	protected Response<Bitmap> parseNetworkResponse(NetworkResponse response) {
		...
		synchronized (sDecodeLock) {
			try {
				...
				//L.e("Caught OOM for %d byte image, url=%s",
