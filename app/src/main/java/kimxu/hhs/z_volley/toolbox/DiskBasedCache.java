...
package kimxu.hhs.z_volley.toolbox;
...
import kimxu.hhs.z_volley.Cache;
...
public class DiskBasedCache implements Cache {
	...
	private static int DEFAULT_DISK_USAGE_BYTES = 5 * 1024 * 1024;
	...
	{
		DEFAULT_DISK_USAGE_BYTES = Integer.parseInt(RuntimeConstant.MEMORY_TYPE
				.getConfig("default_disk_request_buffer_size").toString()) * 1024 * 1024;
	}
	...
	public DiskBasedCache(File rootDirectory) {
		...
		//L.x("disk cache size:", DEFAULT_DISK_USAGE_BYTES);
	}
	...
	public synchronized Entry get(String key) {
		...
		try {
			...
			//L.v("%s: %s", file.getAbsolutePath(), e.toString());
			remove(key);
			return null;
		} finally {
			if (cis != null) {
	...
	public synchronized void initialize() {
		if (!mRootDirectory.exists()) {
			if (!mRootDirectory.mkdirs()) {
				//L.e("Unable to create cache dir %s",
	...
	public synchronized void put(String key, Entry entry) {
		...
		try {
			...
			if (!success) {
				fos.close();
				//L.v("Failed to write header for %s", file.getAbsolutePath());
		...
		if (!deleted) {
			//L.v("Could not clean up file %s", file.getAbsolutePath());
		}
	}
	...
	public synchronized void remove(String key) {
		...
		removeEntry(key);
		if (!deleted) {
			//L.v("Could not delete cache entry for key=%s, filename=%s", key,
...
//				L.v("Could not delete cache entry for key=%s, filename=%s",
//						e.key, getFilenameForKey(e.key));
		...
		// if (VolleyLog.DEBUG) {
//		L.v("pruned %d files, %d bytes, %d ms", prunedFiles,
