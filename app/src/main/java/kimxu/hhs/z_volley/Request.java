...
import kimxu.hhs.z_volley.http.CommonErrListener;
...
import kimxu.hhs.z_volley.http.Recorder;
 ...
 */
public abstract class Request<T> extends GenericRequest<T> implements
	 ...
	 * response.
	 */
	protected Request(int method, String url, CommonResponceListener<T> listener) {
		mMethod = method;
		mUrl = url;
		...
		mLifeEvents = CollectionBuilder.newArrayList(15);
		id = seqMaker.getAndIncrement();
	...
	/**
	 * @return The hashcode of the URL's host component, or 0x1ff if there is
	 *         none.
	 */
	...
	public void addMarker(String tag) {
		...
		mLifeEvents.add(new Recorder(tag, System.currentTimeMillis()));
	}
	...
	void finish(final String tag) {
		...
		if (requestTime >= SLOW_REQUEST_THRESHOLD_MS) {
			//L.e("%d ms: %s", requestTime, this.toString());
			// TODO : record the slow request
//			try {
//				HashMap<String, String> purchase = new HashMap<String, String>();
//				purchase.put("net_work_request_slow",
//						String.valueOf(requestTime));
//				MobclickAgent.onEvent(MApplication.getAppContext(),
//						"net_request_stat", purchase);
//			} catch (Exception ex) {
//
//			}
		}
	...
	private byte[] encodeParameters(Map<String, String> params,
		...
		try {
			for (Map.Entry<String, String> entry : params.entrySet()) {
				...
				String vv = URLEncoder.encode(v == null ? "" : v,
						paramsEncoding);
	...
	/**
	 * Returns the {@link HttpRequestPriority} of this request;
	 * {@link HttpRequestPriority#NORMAL} by default.
	 */
	...
	protected void deliverResponse(T response) {
		try {
//			HashMap<String, String> purchase = new HashMap<String, String>();
//			purchase.put("network_request_info", "success");
//			MobclickAgent.onEvent(MApplication.getAppContext(),
//					"net_request_stat", purchase);
		} catch (Exception ex) {
		...
		if (this.mListener != null) {
			//TODO Volley deliverResponse
			this.mListener.onResponse(response, this);
		}
	}
	...
	public void deliverError(VolleyError error) {
		try {
//			HashMap<String, String> purchase = new HashMap<String, String>();
//			purchase.put("network_request_info", "fail");
//			MobclickAgent.onEvent(MApplication.getAppContext(),
//					"net_request_stat", purchase);
		} catch (Exception ex) {
	...
	public int compareTo(Request<T> other) {
		HttpRequestPriority left = this.getPriority();
		HttpRequestPriority right = other.getPriority();
	...
	public void dumpLifeCycle() {
		//L.f(true, this.mLifeEvents.toString());
	}

	protected synchronized void setListener(CommonResponceListener<T> listener) {
		if (listener == null) {
			mErrorListener = ls;
			mListener = null;
		} else {
	...
	public String getDebugBody() {
		if (AppConstant.isDebug) {
			try {
				...
				// String ss = this.getParams().toString();
				return s;
